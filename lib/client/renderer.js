...
function init (client, d3) {
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    function treatmentTooltip (d) {
      ...
      if (client.settings.units === 'mmol' && d.units === 'mg/dl') {
    ...
    function rectHeight (d) {
      ...
      if (d.targetTop && d.targetTop > 0 && d.targetBottom && d.targetBottom > 0) {
        var tTop = (d.units === 'mmol' ? consts.MMOL_TO_MGDL * d.targetTop :  d.targetTop);
        var tBottom = (d.units === 'mmol' ? consts.MMOL_TO_MGDL * d.targetBottom :  d.targetBottom);
        height = Math.max(5, tTop - tBottom);
      }
      return height;
    }
    ...
    function rectTranslate (d) {
      ...
      if (d.eventType === 'Temporary Target') {
        var tTop = (d.units === 'mmol' ? consts.MMOL_TO_MGDL * d.targetTop :  d.targetTop);
        var tBottom = (d.units === 'mmol' ? consts.MMOL_TO_MGDL * d.targetBottom :  d.targetBottom);
        top = tTop === tBottom ? tTop + rectHeight(d) : tTop;
      }
