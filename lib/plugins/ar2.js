...
function selectEventType (prop, sbx) {
  ...
  if (in20mins !== undefined) {
    if (sbx.settings.alarmHigh && in20mins > sbx.scaleMgdl(sbx.settings.thresholds.bgHigh)) {
      eventName = 'high';
    } else if (sbx.settings.alarmLow && in20mins < sbx.scaleMgdl(sbx.settings.thresholds.bgLow)) {
      eventName = 'low';
    }
  }
