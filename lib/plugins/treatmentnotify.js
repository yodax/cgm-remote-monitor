...
const AUTOMATED_TREATMENTS = ['Temp Basal'];
...
function init(ctx) {
  ...
  function filterTreatments (sbx) {
    ...
    treatments = _.filter(treatments, function notLoopAutomated (treatment) {
      var ok = true;
      let enteredBy = treatment.enteredBy;
      if (enteredBy && (enteredBy.indexOf('openaps://') === 0 || enteredBy.indexOf('loop://') === 0)) {
        ok = !(_.indexOf(AUTOMATED_TREATMENTS, treatment.eventType) >= 0);
      }
      let app = treatment.app;
      if (app && (app.indexOf('AAPS') === 0)) {
        ok = !(_.indexOf(AUTOMATED_TREATMENTS, treatment.eventType) >= 0);
      }
      return ok;
    });
