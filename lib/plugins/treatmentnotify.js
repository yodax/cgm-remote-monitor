...
const MEDTRONIC_AUTOMATED_TREATMENTS = ['Temp Basal'];
...
function init(ctx) {
  ...
  //automated treatments from OpenAPS, Loop or Medtronic shouldn't trigger notifications or snooze alarms
  function filterTreatments (sbx) {
    ...
    treatments = _.filter(treatments, function notMedtronicAutomated (treatment) {
      var ok = true;
      var enteredBy = treatment.enteredBy;
      if (enteredBy && (enteredBy.indexOf('medtronic-600://') === 0)) {
        ok = !(
                _.indexOf(MEDTRONIC_AUTOMATED_TREATMENTS, treatment.eventType) >= 0
                &&
                (treatment.notes.startsWith('microb') || treatment.notes.startsWith('mikrob'))
        );
      }
      return ok;
    });
